...
VERDICTS = {
    ...
    "dev_leisos": ("no",
        "**In plain terms.** Somebody in October 2024 making the code of the "
        "day run on their own GP13 star-shape simulation. They hit three "
        "problems and patched around each. All three were properly solved on "
        "the trunk -- by other people, in different ways, within days or "
        "months. Nothing here is worth taking."
        "\n\n"
        "**Corrects an earlier reading here.** This was listed as a "
        "cherry-pick for \"a real get_antenna_position fix (an `or` that "
        "should be an `and`) buried under 35,000 lines of CORSIKA data\". The "
        "burial is real -- of 34 files, three are Python and the rest is a "
        "committed CORSIKA run directory, four 1.4 MB .rawroot binaries and "
        "Emacs droppings (`#test#`, `test~`) -- but the fix is not."
        "\n\n"
        "**In detail**, all verified."
        "\n\n"
        "*The `or`/`and` change is moot.* Six days before this branch, "
        "lguelzow's 3a28782 (2024-10-09) replaced the hard-coded GP300.list "
        "with the simulation's own list, so `get_antenna_position` now reads "
        "`pathAntennaList` twice and compares a file with itself. The two "
        "filtered arrays are always both empty or both non-empty, the guard "
        "can never half-fire, and `or` and `and` are the same thing. The trunk "
        "says so at CorsikaInfoFuncs.py:273: `# currently checks the same "
        "file, so always passes`. The branch's version also silently flips "
        "which list wins when both match -- coincidentally to the same choice "
        "the trunk made deliberately."
        "\n\n"
        "*The PARALLEL patch is superseded and worse.* It comments the read "
        "out and sets `ECTCUT = ECTMAX = 0`. The trunk's 10f4649 returns None "
        "and writes -1, the converter's \"not available\" sentinel; 0 is a "
        "legitimate value and would be indistinguishable from a measurement."
        "\n\n"
        "*The pipeline fix has nothing to land on.* "
        "sim2root/CoREASRawRoot/coreas_pipeline.py was deleted on 2025-03-17 "
        "by Jelena Köhler, \"This pipeline is not up-to-date anymore\". Its "
        "one genuine improvement -- passing `-f` per file instead of `-d`, "
        "which had every shower converted once per shower in the directory -- "
        "dies with the file. Its other change scrapes sim2root's stdout for a "
        "string sim2root never prints, so the loop never matches and the "
        "variable it sets is never bound."
        "\n\n"
        "*One hunk applies cleanly and must still not be taken.* The "
        "`calculate_array_shift` None-guard grafts onto the current "
        "CoreasToRawROOT.py without complaint, and its success path is "
        "`print(f\"Shift X: {shift_x}, Shift Y: {shift_y}\")` -- names with "
        "**zero definitions in the file**. It raises NameError on exactly the "
        "GP300-shaped simulations the trunk is built for. The author never saw "
        "it because their own data always took the other branch."
        "\n\n"
        "**The lesson, which cost two wrong verdicts.** This and "
        "`masterkastner` were both listed as cherry-picks and both collapsed "
        "on a closer look, for the same reason: a branch's diff shows what it "
        "changed, never whether the destination still has the problem. In both "
        "cases somebody had already fixed it another way, and no comparison "
        "against the merge-base can reveal that."),
...
DECIDED = {
    ...
    "dev_leisos": ("2026-09-08",
        "its three patches were each solved properly on the trunk by other "
        "people -- the antenna-list check six days earlier, PARALLEL today, "
        "and the pipeline file deleted in March 2025; the one hunk that still "
        "applies raises NameError on the success path"),
