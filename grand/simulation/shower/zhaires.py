...
class ZhairesShower(ShowerEvent):
    ...
    def _from_dir(cls, path: Path) -> ZhairesShower:
        ...
        ecef         = ECEF(x=origin[0][0], y=origin[1][0], z=origin[2][0]) # RK. x, y and z are given in ECEF.
        inp['frame'] = LTP(location=ecef, orientation=orientation,
    ...
    def _from_datafile(cls, path: Path) -> ZhairesShower:
        with h5py.File(path, 'r') as fd:
            ...
            for tag, x, y, z, *_ in antennas:
                ...
                t  = numpy.asarray(efield[:,0], 'f8') * 1.e-9 # ns --> s
            ...
            try:
                ...
                geodetic = Geodetic(latitude=latitude, longitude=longitude, height=0.)
                origin   = ECEF(geodetic)
                frame    = LTP(location=origin, orientation='NWU',
                               declination=declination, obstime=obstime)

            return cls(
                energy  = float(event[0, 'Energy']) * 1.e9, # EeV --> GeV
                ...
                maximum = LTP(x=event[0, 'XmaxPosition'][0],
                              y=event[0, 'XmaxPosition'][1],
                              z=event[0, 'XmaxPosition'][2], frame=frame),  # RK TODO: Check if x,y,z are defined in frame.
                fields = fields
            )
