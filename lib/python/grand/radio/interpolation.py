...
XXX / TODO: check for autopep8 mistakes
'''
...
# '''calcs frequencies for rfft, exactly as numpy.fft.rfftfreq, 
#   lacking that function in my old numpy version.
# Parameters:
# ---------
#n: int
# Number of points.
...
def unwrap(phi, ontrue=None):
    ...
    for i0, p1 in enumerate(phi[1:]):
        ...
        if ontrue is not None:
            logger.info(f"{i}, {phi[i]}, {phi[i-1]}, {l}, 
                        {phi_unwrapped[i]}, {abs(phi[i] - phi[i-1])},
                        {abs(phi[i] - phi[i-1] + np.pi)}, 
                        {abs(phi[i] - phi[i-1] - np.pi)}")
    return phi_unwrapped


def interpolate_trace(t1, trace1, x1, t2, trace2, x2, xdes, 
                      upsampling=None,  zeroadding=None,
                      ontrue=None, flow=60.e6, fhigh=200.e6, DISPLAY=False):
    """Interpolation of signal traces at the specific position 
    in the frequency domain

    The interpolation of traces needs as input antenna position 1 and 2, 
    their traces (filtered or not) 
    in one component, their time, and the desired antenna position 
    and returns the trace ( in x,y,z coordinate system) and 
    the time from the desired antenna position.
    Zeroadding and upsampling of the signal are optional functions.

    IMPORTANT NOTE:
    The interpolation of the phases includes the interpolation of 
    the signal arrival time. A linear interpolation implies a plane radio
    emission wave front, which is a simplification as it is hyperbolic in shape. 
    However, the wave front can be estimated as a plane between two simulated 
    observer positions.
    For a sufficiently dense grid of observers, as then parts of the wave 
    front are linear on small scales.

    This script bases on the diploma thesis of Ewa Holt (KIT, 2013) 
    in the context of AERA/AUGER. It is based on the interpolation of 
    the amplitude and the pahse in the frequency domain. 
    This can lead to misidentifiying of the correct phase. 
    We are working on the interplementaion on a more robust interpolation 
    of the signal time.
    Feel free to include it if you have some time to work on it. 
    The script is completely modular so that single parts can be substitute easily.
    ...
    ----------
            ...
            flow: float
                lower frequency - optional, define the frequency range for plotting, 
                if desired (DISPLAY=True/False)
            fhigh: float
                higher frequency - optional, define the frequency range for plotting, 
                if desired (DISPLAY=True/False)
            DISPLAY: True/False
    ...
    # hand over time traces of one efield component -t1=time, trace1=efield- 
    # and the position x1 of the first antenna, 
    # the same for the second antenna t2,trace2, x2.
    # xdes is the desired antenna position (m) where 
    # you would like to have the efield trace in time
    ...
    # since in star shape pattern it is mor a radial function connection 
    # the position of same signal as linear go for that solution.
    ...
    if np.isinf(weight1):
        logger.info(f"weight = inf: {x1}, {x2}, {xdes}")
        weight1 = 1.
        weight2 = 0.
    if np.isnan(weight1):
        info('Attention: projected positions equivalent')
        weight1 = 1.
        weight2 = 0.
    epsilon = np.finfo(float).eps
    if (weight1 > 1. + epsilon) or (weight2 > 1 + epsilon):
        logger.info(f"weight larger 1: 
            {weight1}, {weight2}, {x1}, {x2}, {xdes}, 
            {np.linalg.norm(x2-x1)}, {np.linalg.norm(x2-xdes)}, 
            {np.linalg.norm(xdes-x1)}")
    if weight1 + weight2 > 1 + epsilon:
        logger.info("PulseShape_Interpolation.py: order in simulated positions. 
        Check whether ring or ray structure formed first")
        logger.info(f"{weight1}, {weight2}, {weight1}, {weight2}")
    ...
    if ontrue is not None:
        logger.info(f"{phides}")
    # if DISPLAY:
        #phides2 = phides.copy()

    # re-unwrap: get -pi to +pi range back 
    # and check whether phidesis inbetween
    ...
    # Amp, Amp2
    # Since the amplitude shows a continuous unipolar shape, 
    # a linear interpolation is sufficient
...
def do_interpolation(desired, array, zenith, azimuth, phigeo=phigeo, thetageo=thetageo, 
                     shower_core=np.array([0, 0, 0]), DISPLAY=False):
    '''
    ...
    NOTE: The selection of the neigbours is sufficiently stable, 
    but does not always pick the "best" neigbour, still looking for an idea
    XXX / TODO: Read-in and save only hdf5 files
    '''

    # DESIRED
    # Hand over a list file including the antenna positions 
    # you would like to have.
    # positions[:,0]:x, positions[:,1]:y,positions[:,2]:z
    positions = np.loadtxt(desired)
    if DISPLAY:
        logger.info(f"desired positions: {positions}, {len(positions)}")
    if len(positions) <= 1:
        logger.info(f"Files of desired positions has to consist of
                at least two positions, Bug to be fixed")
    ...
    if DISPLAY:
        logger.info(f"simulated positions: 
                        {positions_sims}, {len(positions_sims)}")
    if len(positions_sims) <= 1:
        logger.info(f"Files of simulated positions has to consist 
                of at least two positions, Bug to be fixed")

    # SELECTION: For interpolation only select the desired position 
    # which are "in" the plane of simulated antenna positions
    ...
    # test wether desired points are in_plane, 
    # needed assumption for interpolation
    ind = []
    ...
    if DISPLAY:
        ...
        #ax.scatter(positions_sims[:,0], positions_sims[:,1], 
        #           positions_sims[:,2], label = "simulated")
        #ax.scatter(positions[:,0], positions[:,1], positions[:,2], 
        #           label = "desired")
        #ax.scatter(shower_core[0], shower_core[1], shower_core[2], 
        #           label = "shower core")
        for j in range(0, len(positions[:, 1])):
            ax.annotate(str(j), ((positions[j, 0], positions[j, 1])))
        ax.scatter(positions_sims[:, 0], positions_sims[:, 1], 
                   label="simulated")
        ax.scatter(positions[ind, 0], positions[ind, 1], 
                   label="desired")
        ax.scatter(shower_core[0], shower_core[1],  
                   label="shower core")
    ...
    # XXX / TODO should be substituded bey line of sight Xmax - positions
    ...
    for i in np.arange(0, len(positions[:, 1])):
        ...
        positions[i, :] = positions[i, :] + b*v - \
            shower_core  # correct by shower core position
    for i in np.arange(0, len(positions_sims[:, 1])):
        b = -np.dot(n, positions_sims[i, :]) / np.dot(n, v)
        positions_sims[i, :] = positions_sims[i, :] + b*v - \
            shower_core  # correct by shower core position

    # START: ROTATE INTO SHOWER COORDINATES, and core for offset 
    # by core position, alreadz corrected in projection
    #GetUVW = UVWGetter(shower_core[0], shower_core[1], shower_core[2], 
    #                   zenith, azimuth, phigeo, thetageo)
    ...
    if DISPLAY:
        ...
        #ax2.scatter(pos_sims[:,0], pos_sims[:,1], pos_sims[:,2], 
        #            label = "simulated")
    ...
    points = []
    # position should be within one plane yz plane, 
    # remove x=v component for simplicity
    for i in np.arange(0, len(pos_sims[:, 1])):
    ...
    # position should be within one plane yz plane, remove x=v component for simplicity
    for i in np.arange(0, len(pos[:, 1])):
        ...
        for m in np.arange(0, len(points)):  # desired points as reference
            ...
            # weighting approach1
            # distance = np.sqrt(delta_r**2 + (delta_r *delta_phi)**2 ) 
            # euclidean distance
            # distance= np.sqrt((pos_sims[m,1]-pos[i,1])**2. +(pos_sims[m,2]-pos[i,2])**2.) 
            # polar coordinates
            distance = np.sqrt(points[m][2]**2. + radius**2. - 2.*points[m][2]
                               *radius * np.cos(points[m][1]-theta)) 

            if delta_phi >= 0. and delta_r >= 0:
                points_I.append((m, abs(round(delta_phi, 2)),
                                 abs(round(delta_r, 2)), distance))
            if delta_phi > 0. and delta_r < 0:
                points_II.append((m, abs(round(delta_phi, 2)),
                                  abs(round(delta_r, 2)), distance))
            if delta_phi <= 0. and delta_r <= 0:
                points_III.append(
                    (m, abs(round(delta_phi, 2)), abs(round(delta_r, 2)), distance))
            if delta_phi < 0. and delta_r > 0:
                points_IV.append((m, abs(round(delta_phi, 2)),
                                  abs(round(delta_r, 2)), distance))

        if not points_I:
            logger.error(
                f"list - Quadrant 1 - empty --> no interpolation for ant {ind[i]}")
            continue
        if not points_II:
            logger.error(
                f"list - Quadrant 2 - empty --> no interpolation for desired ant {ind[i]}")
            continue
        if not points_III:
            logger.error(
                f"list - Quadrant 3 - empty --> no interpolation for ant {ind[i]}")
            continue
        if not points_IV:
            logger.error(
                f"list - Quadrant 4 - empty --> no interpolation for ant {ind[i]}")
            continue

        points_I = np.array(points_I, dtype=[('index', 'i4'), ('delta_phi', 'f4'), 
                                             ('delta_r', 'f4'), ('distance', 'f4')])
        points_II = np.array(points_II, dtype=[('index', 'i4'), ('delta_phi', 'f4'), 
                                               ('delta_r', 'f4'), ('distance', 'f4')])
        points_III = np.array(points_III, dtype=[('index', 'i4'), ('delta_phi', 'f4'), 
                                                 ('delta_r', 'f4'), ('distance', 'f4')])
        points_IV = np.array(points_IV, dtype=[('index', 'i4'), ('delta_phi', 'f4'), 
                                               ('delta_r', 'f4'), ('distance', 'f4')])

        # Sort points; not optimal (the best) solution for all, 
        # but brings stable/acceptable results
        points_I = np.sort(points_I,
                           order=['distance', 'delta_phi', 'delta_r'])
        points_II = np.sort(points_II, 
                            order=['distance', 'delta_phi', 'delta_r'])
        points_III = np.sort(points_III, 
                             order=['distance', 'delta_phi', 'delta_r'])
        points_IV = np.sort(points_IV, 
                            order=['distance', 'delta_phi', 'delta_r'])
        # indizes of 4 closest neigbours: points_I[0][0], points_II[0][0], 
        # points_III[0][0], points_IV[0][0]

        # try to combine the one with roughly the same radius first 
        # and then the ones in phi -- Project Point on line 1 - I-IV
        point_online1 = _ProjectPointOnLine(pos_sims[points_I[0][0]], 
                                            pos_sims[points_IV[0][0]], 
                                            pos[i])
        # Project Point on line 2 - II-III
        point_online2 = _ProjectPointOnLine(pos_sims[points_II[0][0]],
                                            pos_sims[points_III[0][0]], 
                                            pos[i])
        ...
        if DISPLAY:
            ...
            ax3.scatter(pos_sims[points_I[0][0], 1],
                        pos_sims[points_I[0][0], 2], label="1")
            ax3.scatter(pos_sims[points_II[0][0], 1],
                        pos_sims[points_II[0][0], 2], label="2")
            ax3.scatter(pos_sims[points_III[0][0], 1],
                        pos_sims[points_III[0][0], 2], label="3")
            ax3.scatter(pos_sims[points_IV[0][0], 1],
                        pos_sims[points_IV[0][0], 2], label="4")
        ...
        # XXX / TODO: read-in table instead textfile
        directory = split(array)[0]+"/"
        logger.info(f"Read traces from {directory}")

        txt0 = load_trace(directory, points_I[0][0], suffix=".trace")
        txt1 = load_trace(directory, points_IV[0][0], suffix=".trace")
        xnew1, tracedes1x = interpolate_trace(txt0.T[0], txt0.T[1], positions_sims[points_I[0][0]], 
                                              txt1.T[0], txt1.T[1], positions_sims[points_IV[0][0]], 
                                              point_online1, upsampling=None, zeroadding=None)
        xnew1, tracedes1y = interpolate_trace(txt0.T[0], txt0.T[2], positions_sims[points_I[0][0]], 
                                              txt1.T[0], txt1.T[2], positions_sims[points_IV[0][0]], 
                                              point_online1, upsampling=None, zeroadding=None)
        xnew1, tracedes1z = interpolate_trace(txt0.T[0], txt0.T[3], positions_sims[points_I[0][0]], 
                                              txt1.T[0], txt1.T[3], positions_sims[points_IV[0][0]], 
                                              point_online1, upsampling=None, zeroadding=None)

        txt2 = load_trace(directory, points_II[0][0], suffix=".trace")
        txt3 = load_trace(directory, points_III[0][0], suffix=".trace")
        xnew2, tracedes2x = interpolate_trace(txt2.T[0], txt2.T[1], positions_sims[points_II[0][0]], 
                                              txt3.T[0], txt3.T[1], positions_sims[points_III[0][0]], 
                                              point_online2, upsampling=None, zeroadding=None)
        xnew2, tracedes2y = interpolate_trace(txt2.T[0], txt2.T[2], positions_sims[points_II[0][0]], 
                                              txt3.T[0], txt3.T[2], positions_sims[points_III[0][0]], 
                                              point_online2, upsampling=None, zeroadding=None)
        xnew2, tracedes2z = interpolate_trace(txt2.T[0], txt2.T[3], positions_sims[points_II[0][0]], 
                                              txt3.T[0], txt3.T[3], positions_sims[points_III[0][0]], 
                                              point_online2, upsampling=None, zeroadding=None)

        # Get the pulse shape of the desired position from projection on line1 and 2
        xnew_desiredx, tracedes_desiredx = interpolate_trace(xnew1, tracedes1x, point_online1, 
                                                             xnew2, tracedes2x, point_online2, 
                                                             positions[ind[i]], zeroadding=None)
        xnew_desiredy, tracedes_desiredy = interpolate_trace(xnew1, tracedes1y, point_online1, 
                                                             xnew2, tracedes2y, point_online2, 
                                                             positions[ind[i]], zeroadding=None)
        xnew_desiredz, tracedes_desiredz = interpolate_trace(xnew1, tracedes1z, point_online1, 
                                                             xnew2, tracedes2z, point_online2, 
                                                             positions[ind[i]], zeroadding=None)

        # TODO Save as hdf5 file instead of textfile
        logger.info(f"Interpolated trace stord as {split(desired)[0]} /a{ind[i])}.trace")
...
def main():
    ...
    # path to list of desied antenna positions, 
    # traces will be stored in that corresponding folder
    #desired  = sys.argv[1]
    ...
    # call the interpolation: Angles of magnetic field 
    # and shower core information needed, 
    # but set to default values
    do_interpolation(desired, array, zenith, azimuth, phigeo=phigeo,
                     thetageo=thetageo, shower_core=np.array([0, 0, 2900]), 
                     DISPLAY=False)
