...
def digitization(voltages, tsampling=tsampling):
    ...
    # voltages.T[0, 1] - voltages.T[0, 0]
    dt = round(np.mean(np.diff(voltages.T[0])))
...
def standard_processing(efield, zenith_sim, azimuth_sim, alpha_sim=0., beta_sim=0.,
    ...
    if 'antennaresponse' in processing:
        ...
        # XXX /TODO from invert_computevoltage import compute_electicfield
    ...
    if 'noise' in processing:
        ...
        # remove tranposed in signal_treatment
        trace = add_noise(trace, vrms=Vrms2)
    ...
    if 'filter' in processing:
        ...
        if DISPLAY == 1:
            ...
            freqs = np.fft.rfftfreq(
                len(trace.T[1]), trace.T[0, 1]-trace.T[0, 0])
