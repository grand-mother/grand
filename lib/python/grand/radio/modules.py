...
def _dist_decay_Xmax(zen, injh2, Xmax_primary):
    ...
    while X < Xmax_primary:
        i = i+1
        ai = i*step  # 100. #m
        hi = -Re+np.sqrt(Re**2. + ai**2. + hD**2. + 2. * Re * hD - \
                         2 * ai * np.cos(gamma) * (Re+hD))  # cos(gamma)= + to - at 90dg
        deltah = abs(h-hi)  # (h_i-1 - hi)= delta h
        h = hi  # new height
        # Xmax in g/cm2, slanted = Xmax, vertical/ cos(theta); 
        # density in g/cm3, h: m->100cm, np.pi-zen2 since it is defined 
        # as where the showers comes from, abs(cosine) so correct for minus values
        X = X + rho_0*np.exp(-g*M*hi/(R*T)) * step*100.  # (deltah*100) *abs(1./np.cos(np.pi-zen2)) 
...
def _get_XmaxPosition(primary, energy, zen, azim, injh=0, GdAlt=0, core=np.array([0., 0., 0.])):
    ...
    u_sh = np.array([np.cos(azimr) * np.sin(zenr), 
                     np.sin(azimr) * np.sin(zenr), 
                     np.cos(zenr)])
    ...
    if primary == "proton" or primary == "iron":
        ...
        # use some trigonmetrie, 
        # currently assuming ***(0,0,0) as origin***, Groundaltitude = 0m
        # a = Re
        # c = Re + Ra
        ...
        # XXX / TODO: allow a shower_core != (0,0,0)
        # with b as the full slanted distance to top atmosphere at that zenith
        # new = float(ai -b) *u_sh 
        new = float(ai) * -u_sh
...
def _get_CRzenith(zen, injh, GdAlt):
    ''' Corrects the zenith angle for CR respecting Earth curvature, 
        zenith seen by observer
    ...
    a = np.sqrt((Re + injh)**2. - \
                (Re+GdAlt)**2 * np.sin(np.pi - np.deg2rad(zen))**2) - \
        (Re+GdAlt)*np.cos(np.pi-np.deg2rad(zen))
    zen_inj = np.rad2deg(np.pi-np.arccos((a**2 + (Re+injh)**2 - Re**2)
                                         /(2*a*(Re+injh))))
