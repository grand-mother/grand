#! /usr/bin/env python
from grand import Coordinates, CartesianRepresentation, SphericalRepresentation, GeodeticRepresentation, topography
from grand import ECEF, Geodetic, GRANDCS, LTP
from grand import Geomagnet
from grids import create_grid_univ  # GRAND_tools rep https://github.com/kumikokotera/GRAND_tools. Add GRAND_tools/grid_shape rep to PYTHONPATH
...
X, Y = np.meshgrid(x, y)
coordinates = GRANDCS(x=X.flatten(),  y=Y.flatten(), z=np.zeros(X.size),location=dunhuang) # CS: x = North, y = West
# Get the local ground elevation. Note that local coordinates naturally account for the Earth curvature.
zg = topography.elevation(coordinates, reference="local") # Altitude measured wrt cart ref centered on origin "location"
...
## Detector
# Generate grid
pos_hex, offset2=create_grid_univ(GridShape='hexhex',radius=816,angle=0,do_offset=False, Nrand=None,randeff=None,DISPLAY=False,directory=None,do_prune=False,input_n_ring=3)
det_cs = GRANDCS(x=pos_hex[0,:].flatten(),  y=pos_hex[1,:].flatten(), z=np.zeros(pos_hex[0,:].size),location=dunhuang)
...
for i,_ in enumerate(det_cs.x):
   tofile = " A" + str(i) +  " " + "{:.1f}".format(det_cs.x[i]) + " " + "{:.1f}".format(det_cs.y[i]) + " " + "{:.6f}".format(det_geo.latitude[i]) + " " + "{:.6f}".format(det_geo.longitude[i]) + " " + "{:.1f}".format(det_cs.z[i])
   print(tofile)
   antpos.write(tofile+'\n')
antpos.close()
